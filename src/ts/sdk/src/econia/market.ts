...
export class OrderBook
{
  ...
  price_levels_vectors(
    ...
    const cache = this.__app?.cache || new AptosLocalCache();
    const tags = (this.typeTag as StructTag).typeParams;
    return price_levels_vectors(this, cache);
  }
...
export function cancel_limit_order_ (
  ...
  if (!Critbit.has_key(temp$2, temp$3, $c, [new SimpleStructTag(Order)])) {
    throw $.abortCode($.copy(E_NO_ORDER));
  }
  let { user: order_user, general_custodian_id: order_general_custodian_id } = Critbit.pop(tree_ref_mut, $.copy(order_id), $c, [new SimpleStructTag(Order)]);
  ...
  if (temp$4) {
    if (Critbit.is_empty(tree_ref_mut, $c, [new SimpleStructTag(Order)])) {
      temp$5 = $.copy(MIN_ASK_DEFAULT);
    }
    else{
      temp$5 = Critbit.min_key(tree_ref_mut, $c, [new SimpleStructTag(Order)]);
    }
    ...
    if (temp$6) {
      if (Critbit.is_empty(tree_ref_mut, $c, [new SimpleStructTag(Order)])) {
        ...
        temp$7 = Critbit.max_key(tree_ref_mut, $c, [new SimpleStructTag(Order)]);
      }
...
export function match_init_ (
  ...
  n_orders = Critbit.length(tree_ref_mut, $c, [new SimpleStructTag(Order)]);
...
export function match_loop_break_ (
  ...
  if ($.copy(should_pop_last_ref)) {
    Critbit.pop(tree_ref_mut, $.copy(final_order_id_ref), $c, [new SimpleStructTag(Order)]);
  }
  else{
  }
  return;
...
export function match_loop_init_ (
  ...
  [target_order_id, target_order_ref_mut, target_parent_index, target_child_index] = Critbit.traverse_init_mut(tree_ref_mut, $.copy(traversal_direction_ref), $c, [new SimpleStructTag(Order)]);
...
export function match_loop_order_follow_up_ (
  ...
  target_order_ref_mut = Critbit.borrow_mut(tree_ref_mut, $.copy(target_order_id), $c, [new SimpleStructTag(Order)]);
  ...
  if (($.copy(n_orders_ref_mut)).eq((u64("1")))) {
    ...
    if ($.copy(complete_target_fill_ref)) {
      [target_order_id, target_order_ref_mut, target_parent_index, target_child_index, empty_order] = Critbit.traverse_pop_mut(tree_ref_mut, $.copy(target_order_id), $.copy(target_parent_index_ref_mut), $.copy(target_child_index_ref_mut), $.copy(n_orders_ref_mut), $.copy(traversal_direction_ref), $c, [new SimpleStructTag(Order)]);
...
export function orders_vector_ (
  ...
  remaining_traversals = (Critbit.length(tree_ref_mut, $c, [new SimpleStructTag(Order)])).sub(u64("1"));
  [target_id, target_order_ref_mut, target_parent_index, ] = Critbit.traverse_init_mut(tree_ref_mut, traversal_direction, $c, [new SimpleStructTag(Order)]);
  while (true) {
    ...
    [target_id, target_order_ref_mut, target_parent_index, ] = Critbit.traverse_mut(tree_ref_mut, $.copy(target_id), $.copy(target_parent_index), traversal_direction, $c, [new SimpleStructTag(Order)]);
...
export function place_limit_order_post_match_ (
  ...
  Critbit.insert(tree_ref_mut, $.copy(order_id), new Order({ size: $.copy(size_to_fill), user: $.copy(user_ref), general_custodian_id: $.copy(general_custodian_id_ref) }, new SimpleStructTag(Order)), $c, [new SimpleStructTag(Order)]);
  return;
}
...
export function price_levels_vectors (
  order_book_ref_mut: OrderBook,
  $c: AptosDataCache,
...
export function register_market_generic (
  host: HexString,
  lot_size: U64,
  tick_size: U64,
  ...
  register_market(host, $.copy(lot_size), $.copy(tick_size), Registry.custodian_id_(generic_asset_transfer_custodian_id_ref, $c), $c, [$p[0], $p[1]]);
  return;
}

export function register_market_pure_coin (
  host: HexString,
  lot_size: U64,
  tick_size: U64,
  $c: AptosDataCache,
  ...
  register_market(host, $.copy(lot_size), $.copy(tick_size), $.copy(PURE_COIN_PAIR), $c, [$p[0], $p[1]]);
  return;
}
...
export function register_order_book_ (
  ...
  Open_table.add_(order_books_map_ref_mut, $.copy(market_id), new OrderBook({ base_type_info: Stdlib.Type_info.type_of_($c, [$p[0]]), quote_type_info: Stdlib.Type_info.type_of_($c, [$p[1]]), lot_size: $.copy(lot_size), tick_size: $.copy(tick_size), generic_asset_transfer_custodian_id: $.copy(generic_asset_transfer_custodian_id), asks: Critbit.empty($c, [new SimpleStructTag(Order)]), bids: Critbit.empty($c, [new SimpleStructTag(Order)]), min_ask: $.copy(MIN_ASK_DEFAULT), max_bid: $.copy(MAX_BID_DEFAULT), counter: u64("0") }, new SimpleStructTag(OrderBook)), $c, [AtomicTypeTag.U64, new SimpleStructTag(OrderBook)]);
  return;
}
