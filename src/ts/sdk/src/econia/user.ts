...
export function fill_order_update_market_account_ (
  ...
  if (complete_fill) {
    Critbit.pop(tree_ref_mut, $.copy(order_id), $c, [AtomicTypeTag.U64]);
  }
  else{
    order_size_ref_mut = Critbit.borrow_mut(tree_ref_mut, $.copy(order_id), $c, [AtomicTypeTag.U64]);
...
export function get_n_orders_internal_ (
  ...
  tree_ref = temp$1;
  return Critbit.length(tree_ref, $c, [AtomicTypeTag.U64]);
}
...
export function get_order_id_nearest_spread_internal_ (
  ...
  tree_ref = temp$1;
  if (!!Critbit.is_empty(tree_ref, $c, [AtomicTypeTag.U64])) {
    throw $.abortCode($.copy(E_NO_ORDERS));
  }
  if ((side == $.copy(ASK))) {
    temp$2 = Critbit.min_key(tree_ref, $c, [AtomicTypeTag.U64]);
  }
  else{
    temp$2 = Critbit.max_key(tree_ref, $c, [AtomicTypeTag.U64]);
  }
  return temp$2;
}
...
export function register_market_account (
  user: HexString,
  market_id: U64,
  general_custodian_id: U64,
  $c: AptosDataCache,
...
export function register_market_accounts_entry_ (
  ...
  Open_table.add_(market_accounts_map_ref_mut, $.copy(market_account_id), new MarketAccount({ base_type_info: Stdlib.Type_info.type_of_($c, [$p[0]]), quote_type_info: Stdlib.Type_info.type_of_($c, [$p[1]]), generic_asset_transfer_custodian_id: $.copy(generic_asset_transfer_custodian_id), asks: Critbit.empty($c, [AtomicTypeTag.U64]), bids: Critbit.empty($c, [AtomicTypeTag.U64]), base_total: u64("0"), base_available: u64("0"), base_ceiling: u64("0"), quote_total: u64("0"), quote_available: u64("0"), quote_ceiling: u64("0") }, new SimpleStructTag(MarketAccount)), $c, [AtomicTypeTag.U128, new SimpleStructTag(MarketAccount)]);
  return;
}
...
export function register_order_internal_ (
  ...
  Critbit.insert(tree_ref_mut, $.copy(order_id), $.copy(size), $c, [AtomicTypeTag.U64]);
...
export function remove_order_internal_ (
  ...
  size = Critbit.pop(tree_ref_mut, $.copy(order_id), $c, [AtomicTypeTag.U64]);
